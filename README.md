...
re-runs the script every midnight UK time and commits the refreshed snapshot.
...
| Automation | `.github/workflows/leaderboard.yml` |

## Setup

1. **Add the API key.** Repo → Settings → Secrets and variables → Actions → New
   repository secret, named `RAINBET_API_KEY`.
2. **Turn on Pages.** Repo → Settings → Pages → Source: **GitHub Actions**.
3. Push, or run the workflow manually from the Actions tab.

Without the secret the workflow still deploys, it just serves the last committed
snapshot instead of refreshing it.
...
year-round in both GMT and BST; the other run is a harmless no-op refresh. To refresh
more often, add another `cron` entry to `.github/workflows/leaderboard.yml`.
